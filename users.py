...
def add_user(user: User):
    ...
    if user.login in users:
        raise HTTPException(status_code=400, detail="Такий логін вже існує")

    users[user.login] = user.dict()
    save_users(users)
    return {"message": "Користувача додано успішно", "data": user}

@app.put("/user/edit/{login}")
def add_user(login: str, new_data: User):
    users = load_users()
    if login in users:
        raise HTTPException(status_code=400, detail="Користувача знайдено")
